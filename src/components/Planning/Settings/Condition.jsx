...
import {get_conditions} from "../../../redux/actions/management";
import {map, size} from "lodash";
import Skeleton from "react-loading-skeleton";

const Condition = () => {
    const payload = useSelector(state => state.Management.conditions)
    ...
    useEffect(() => {
        dispatch(get_conditions(formData))
    }, [formData]);


    return (<Settings>
        <h1 className={"text-gray-800 flex w-full p-2 text-xs md:text-lg "}>Condición</h1>
