...
import {get_packings} from "../../../redux/actions/management";
import {map, size} from "lodash";
import Skeleton from "react-loading-skeleton";

const PackingItems = () => {

    const payload = useSelector(state => state.Management.packings)
    ...
    useEffect(() => {
        dispatch(get_packings(formData))
    }, [formData]);


    return (<Settings>
        <h1 className={"text-gray-800 flex w-full p-2 text-xs md:text-lg "}>Empaques</h1>
