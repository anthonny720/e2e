...
import {get_categories} from "../../../redux/actions/management";
import {map, size} from "lodash";
import Skeleton from "react-loading-skeleton";

const Categories = () => {
    const payload = useSelector(state => state.Management.categories)
    ...
    useEffect(() => {
        dispatch(get_categories(formData))
    }, [formData]);


    return (<Settings>
        <h1 className={"text-gray-800 flex w-full p-2 text-xs md:text-lg "}>Categorias</h1>
