...
import {get_families} from "../../../redux/actions/management";
import {map, size} from "lodash";
import Skeleton from "react-loading-skeleton";

const Family = () => {

    const payload = useSelector(state => state.Management.families)
    ...
    useEffect(() => {
        dispatch(get_families(formData))
    }, [formData]);


    return (<Settings>
        <h1 className={"text-gray-800 flex w-full p-2 text-xs md:text-lg "}>Familia</h1>
