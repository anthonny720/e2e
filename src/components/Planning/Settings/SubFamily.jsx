...
import {get_subfamilies} from "../../../redux/actions/management";
import {map, size} from "lodash";
import Skeleton from "react-loading-skeleton";

const SubFamily = () => {

    const payload = useSelector(state => state.Management.subfamilies)
    ...
    useEffect(() => {
        dispatch(get_subfamilies(formData))
    }, [formData]);


    return (<Settings>
        <h1 className={"text-gray-800 flex w-full p-2 text-xs md:text-lg "}>Subfamilia</h1>
