...
const Form = ({close, data}) => {
    ...
    return (<form className="bg-white  rounded px-8 pt-6 pb-8 mb-4">
        ...
        <div>
            <p className="text-[10px]  font-extralight leading-none text-blue-400 mt-2">Rol:</p>
            <select onChange={(value) => formik.setFieldValue('role', value.target.value)}
                ...
                <option value={"PLANIFICACION_PRODUCCION"}>{"Planner Producción"}</option>
                <option value={"PLANIFICACION_LOGISTICA"}>{"Planner Logística"}</option>
                <option value={"COMPRAS"}>{"Compras"}</option>
                <option value={"ACOPIO"}>{"Acopio"}</option>
                <option value={"CONTABILIDAD"}>{"Contabilidad"}</option>
                <option value={"GERENCIA"}>{"Gerencia"}</option>
                <option value={"COMERCIAL"}>{"Comercial"}</option>
                <option value={"PLANIFICACION_ANALISTA"}>{"Analista de Datos"}</option>
            </select>
        </div>
        <div>
            <p className="text-[10px]  font-extralight leading-none text-blue-400 mt-2">Permiso:</p>
            <select onChange={(value) => formik.setFieldValue('permissions', value.target.value)}
                    value={formik.values.permissions} className="mt-4 form-select appearance-none block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300       rounded transition       ease-in-out
                    m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                    aria-label="Default select example">
...
const newSchema = (data) => {
    return {
        ...
        permissions: Yup.string().required(true),
    }
}
